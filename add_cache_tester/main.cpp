...
int main( const int argc, const char *argv[] )
{
	...
	IceBRG::multi_vector<double> z1 = z1_vals;
	IceBRG::multi_vector<double> z2 = z2_vals;

	IceBRG::multi_vector<double>::shape_t results_shape;

	results_shape.push_back(z1.size());
	results_shape.push_back(z2.size());

	IceBRG::multi_vector<double> results(results_shape,0);

	for(size_t z1_i=0; z1_i<z1.size(); ++z1_i)
	{
		for(size_t z2_i=0; z2_i<z2.size(); ++z2_i)
		{
			IceBRG::multi_vector<double>::shape_t position;
			position.push_back(z1_i);
			position.push_back(z2_i);
			results(position) = IceBRG::value_of(IceBRG::ad_distance(z1[z1_i],z2[z2_i]));
		}
	}
	...
	IceBRG::print_table(std::cout,data);

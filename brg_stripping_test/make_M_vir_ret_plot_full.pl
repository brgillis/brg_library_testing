...
$cmd = "stilts plot2d ofmt=eps out='M_vir_ret_full.eps' legend=true " .
	"grid=false ylog=false xpix=500 ypix=400 ylo=-0.8 ";

for($i=0;$i<=9;$i++) {
    $cmd = $cmd . "ifmt".$i."=ascii in".$i."=stripping_test_orbital".$i.".dat " .
	"cmd".$i."='addcol log\\(Mvir/Mvir,init\\) log10(m_vir_ret)' ydata".$i."=log\\(Mvir/Mvir,init\\) " .
...
for($i=0;$i<=2;$i++) {
	...
	"cmd9".$i."='addcol log\\(Mvir/Mvir,init\\) log10(m_vir_ret)' ydata9".$i."=log\\(Mvir/Mvir,init\\) " .
